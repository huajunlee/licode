...
1. 面向自主推理与实时交互，构建双事件通道 Agent 引擎，将请求编排与界面流式刷新解耦至独立通道互不阻塞，并设步数、token、时长三重上限防止死循环与资源耗尽。

2. 为解决跨会话遗忘与长对话膨胀，建立分层记忆模型，按需主动召回、无关问题不召回而相关问题精准命中，后台整理去重归档以防堆积与矛盾，长对话按轮次结构压缩且全文可恢复。
