...
**构建双通道 Agent 引擎**：面向自主推理与实时交互，将请求编排与界面流式刷新解耦至独立通道互不阻塞，并设步数、token、时长三重上限防止死循环与资源耗尽。

这个项目的核心是一个自主推理的 Agent，最初遇到的一个问题是推理循环和界面刷新是单通道耦合的，模型每吐一个 token 都要等渲染，工具调用期间界面还会卡住。所以我把系统拆成了双通道：主通道参考 KOA 中间件的洋葱模型，是一条中间件流水线。使用洋葱模型的目的是能够在核心循环节点前后插可插拔的节点，也就是类似hook机制的实现。这些节点依次是 before 钩子 → Agent Loop → after 钩子，每个中间件可以放行事件、也可以拦截。每个中间件调用 await next() 就把事件往链深处传，并等链深处所有中间件执行完再回来；不调用 next 就是拦截，事件到此为止，agent loop中间件末尾的错误兜底就是这么兜住 error 事件的，不让它继续往下传。副通道是一个事件总线，Agent Loop 在推理过程中异步往外发事件，界面订阅后做逐字流式刷新，两条通道互不阻塞。另外我给 Agent Loop 加了三重熔断上限——50 步、20 万 token、10 分钟，任一触发就终止循环并返回已有结果，分别防模型重复调工具的死循环、上下文爆炸和任务卡死。最终实现流畅的流式体验，同时 Agent 长时间运行也不会失控。



**建立分层记忆模型**：为解决跨会话遗忘与长对话膨胀，按需主动召回、无关问题不召回而相关问题精准命中，后台整理去重归档以防堆积与矛盾，长对话按轮次结构压缩且全文可恢复。

记忆我是区分了长期和短期记忆。

短期记忆也可以叫做是工作记忆，主要是对上下文的管理。为了避免上下文的token因为大工具的输入导致膨胀，我设置了如果一个工具它的反馈结果大于64KB，那么直接将它存入磁盘，并且只将这个工具它的一个文件指针 + 头部预览送入到上下文当中。当上下文token开始膨胀的时候，达到了设定的85%的阈值时，就会开始进行上下文压缩。我的压缩采用的是一个压缩摘要的方法。首先先去判断了上下文当中哪一些是不可重获的消息，以轮次为界限，让模型针对每一轮去判断它的重要程度。如果是用户的纠偏，或者是模型的一个决策的话，就要保留下来，并且最近两轮内的消息也要保留下来。如果是可以重获的消息，将它压缩成摘要，压缩掉的全文通过Git Blob的对象存储在磁盘上。比如说read方法读到的返回结果，或者是grap方法搜索到的关键词等等。具体而言如下：
...
对长期记忆主要是做了四部分的工作，生产、召回、整理、归档。核心是渐进式披露。我创建了一个记忆索引文件，这记忆索引文件只会标记每一个记忆文件的索引，包含了这个文件它在当前文件夹下的哪个位置，以及某个记忆文件它的一个front matter当中的简略描述。

首先先说说生产，在我的项目里，将长期记忆区分为了四类，user也就是用户画像，feedback用户主动的一个纠偏反馈，Project有时效性的各种目标、项目理解，reference是外部的引用，比如说一些用户收藏的文章网址。然后生产分为了两种方式，一种是用户主动提到记住某项记忆，另一种是在Agent Loop这个核心循环的中间件之后的Hook，触发一个模型后台的异步自动提取，自动提取设计了5分钟冷却，避免每一轮对话都会进行一次自动提取，浪费token。

再说说召回，召回也是分为了两种方式。一种是在agent loop中间件里，当用户提出问题之后，它仅根据用户的问题，触发一个side-call模型的前置召回。另一种方式是，我构建了一个能够召回记忆的工具。在React的过程当中，模型阅读到摘要的记忆文件，它如果想主动的召回某一个记忆，那么它就可以通过这个工具来召回。为了避免前置召回和主动召回之间出现重复召回同一个记忆的情况，我设置了一个注册表【哈希表】，用于标记目前已经召回的记忆。另外由于前置召回的记忆是仅根据用户提出的问题而召回的，因此很容易出现用户问领域完全不同的问题时，旧记忆的冗余导致模型生成出现偏差。因此在每一轮React之前，都会清除掉上一轮所前置召回的所有记忆。而模型主动召回的记忆，它是根据了模型现有的一个上下文所召回的记忆。因此，这个记忆是对全部上下文有指导作用的，不会被清除。

最后再说说整理和归档。整理记忆的触发条件是距离上一次整理超过了24小时，并且新增了5个会话。整理记忆的作用就是去整理已有的记忆当中出现了重复或者是矛盾的记忆。打个比方，模型可能在同一个记忆文件当中存储了语义重复了的语言，或者是用户在上一周说自己喜欢吃蛋挞，然后这一周又说自己不喜欢吃了。整理分为四步，审查所有记忆列出怀疑点、用关键词 grep 最近会话收集证据、模型输出增删改操作同时会归档最近一个月内没有被召回的陈旧记忆、最后重建索引。



**设计结构化日记系统**：读入用户口述日记，由独立模型解析为事实、决定、情绪、人物与候选记忆五类字段，再按重要性与可提升性双维度门槛分流至自动入库、归档人物档案或人审。

在Agent当中，我提供了一个日记工具，解决"Agent 不了解用户生活"的问题。当用户执行运行这个工具的时候，会进入一个日记模式。此时用户可以输入自己想要记录的日记。模型在后台首先会将日记当中涉及到的时间全部转化为绝对时间，避免出现时间上的误差。其次，模型会将用户的日记进行结构化成JSON schema，再和原文进行存储，区分为了**事实、决定、情绪、人物、候选记忆**。其中的候选记忆是指从日记当中提取的可能可以晋升为记忆的重要内容。候选记忆有两个维度，重要性和必要性。重要性是指模型认为这个记忆是否重要，它是否是用户的一个长期目标，决策。必要性指的是这条从日记当中提取的内容是否有必要晋升为长期记忆？模型在生成结构化日记时，会对这两个维度进行评级。当重要性为high，并且必要性不等于low的并且记忆属于偏好/决定/目标三类时候，会直接调用记忆生产的函数，将这段候选内容直接晋升为长期记忆。

并且Agent还提供了主动召回日记的工具。这个工具仅当用户直接提问说我的某一天或者是上个周发生了些什么，才会主动调用。Agent能直接提取到结构化的JSON日记，并且从其中引用到日记的原文，回答用户问题。



**实现决策顾问与规划反思**：汇聚历史决定、事实、人物档案与近期日记为决策上下文，帮用户做有依据、知边界的决策；证据不足则坦诚降级以杜绝编造，复杂决策先规划、再经独立模型隔离反思补漏，两轮收敛后执行。

普通 Agent 给建议是凭空拍脑袋——既不了解用户，也不承认自己不知道。针对这个问题，我做了一个分级决策流程。
主模型按复杂信号分流：简单决策——二选一、低风险、上下文够用——直接调决策工具；复杂决策命中五个信号之一就走规划流程：多维度权衡、高风险难撤销、需要定向召回信息、选项三个以上、影响周期长。
无论简繁，工具都会先从用户的第二大脑汇聚决策上下文：按话题匹配的历史决定、相关事实、相关人物档案和近期日记，让建议有据可依。回答分两式：证据充足时给两三条路径、各列利弊，最后给倾向性建议；证据不足时坦诚降级——把事实和各方立场摆清，明说"目前信息不足"，把判断权交还用户，绝不硬编模糊答案。
复杂决策多一个规划反思环节：主模型先生成结构化计划——维度拆成方面加评估目标、选项、执行步骤、相关人物带关系——然后由一个独立小模型做隔离评审：它只看计划文本、看不到主模型的推理过程，避免附和；评审只报实质遗漏，不通过就带着反馈修订重评，最多两轮收敛，防止无限循环。
最后保存是两步确认：必须用户明确同意，才把这次决策写入日记模块，不污染长期记忆。

```
五个复杂信号（decide-plan.ts:56 + user-guide.md §22 / 行 1279-1287）：

┌────────────────────┬───────────────────────────────────────┐
│        信号        │                 含义                  │
├────────────────────┼───────────────────────────────────────┤
│ 多维度权衡         │ 2+ 竞争维度（钱 vs 成长 vs 家庭）     │
├────────────────────┼───────────────────────────────────────┤
│ 高 stakes / 难撤销 │ 职业、大额支出、重大关系              │
├────────────────────┼───────────────────────────────────────┤
│ 信息不足需定向召回 │ 当前上下文答不好，要跨多主题/人物召回 │
├────────────────────┼───────────────────────────────────────┤
│ 多选项             │ 3+ 真正可行选项，非二选一             │
├────────────────────┼───────────────────────────────────────┤
│ 长影响周期         │ 月/年级                               │
