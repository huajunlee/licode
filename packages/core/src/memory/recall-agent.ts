...
function buildAgentPrompt(richIndex: string, maxResults: number): string {
  return [
    ...
    "",
    "## 工作方式",
    "0. 首要原则：只召回与查询主题/意图相关的记忆，宁可漏选（SELECTED: none），不可错选无关记忆。注意查询常由『主题 + 具体探针』两部分组成（如『用户的音乐偏好，是否有关于孙燕姿的记忆』——主题是音乐偏好，孙燕姿是探针）。判定相关性以主题为准，不以探针值是否在索引中逐字命中为准；索引中无该探针值不代表该主题没有记忆，存在同主题记忆仍应选中，以便主模型据此合并更新；读后仍拿不准的，不选。",
