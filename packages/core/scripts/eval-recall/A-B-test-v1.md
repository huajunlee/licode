...
**结论**：指代消解指引若要落地，**工具 schema 与主 system prompt 两个位置都试过、都压制触发**——deepseek-chat 只要读到任何"处理代词/指代"的指令就变保守（把工具误读成"专用于消代词的"），无代词但需要记忆的查询（a2 回复风格/a5 默认模型/a7 戒糖）就漏触发。唯一未试的路径是**与主模型指令解耦的结构化消解**：给子代理传入最近对话上下文让它自己解指代（子代理侧），或一个确定性的前置消解步骤——待定，不在本次提交范围。当前保持纯原样（验证 A=100%/误召回 0%）。
