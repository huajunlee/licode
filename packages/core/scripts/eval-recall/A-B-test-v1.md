...
| query 参数描述 | A 组召回（deepseek-chat，单轮） |
|---|---|
| "直接传入用户的原始问题…"（1.6 回退目标） | **100%**（rounds 1/2/5） |
| 加指代指引（仅参数） | **82%**（round 4：a5/a7 漏） |
| 加指代指引（描述结尾"仅消解代词指代" + 参数） | **64%**（round 3：a2/a5/a7/a9 漏） |
...
**结论**：指代消解指引若要落地，不能放工具 schema（触发决策会读它），需放到工具 schema 之外（如主 system prompt 稳定层，或作为子代理侧对查询的再处理）——待定，不在本次提交范围。
